...
class OQBoostClassifier(BaseEstimator, ClassifierMixin):
    ...
    per node: a full axis scan plus a Deterministic Gradient Covariance Scan
    (DGCS) that derives oblique directions analytically from the gradient-
    feature covariance (w* ∝ Σ_i x_i g_i, the closed-form maximizer of the
    second-order split-gain proxy). No RNG in candidate generation — the old
    stochastic families (GG-SRP random blocks, parent A/B mutation, cache
    crossover) are removed. Lazy best-first growth, depth-adaptive budget.
    ...
    goss : bool
        Gradient-based One-Side Sampling for per-round row subsampling
        (keep top-gradient rows, subsample the rest with rescaling).
        Independent of split-direction generation.
    """
